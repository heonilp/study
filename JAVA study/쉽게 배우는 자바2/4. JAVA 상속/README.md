...
- 자식클래스에서 super클래스를 쓰게 되면 접근 권한이 부여된 부모클래스의 변수와 메소드에 접근할 수 있기 떄문에 밖에서 수정을 안해도되니 캡슐화, 보안을 좀더 높일 수 있다고 생각한다.
...
우리가 인스턴스를 생성할 때, 같은 클래스의 자료형을 가진 변수를 선언하여 인스턴스를 생성해 왔습니다.
...
- 상속으로 가능하다. 자식이 부모의 성질을 다 물려받기 때문에 메소드를 사용할 수 있다.
